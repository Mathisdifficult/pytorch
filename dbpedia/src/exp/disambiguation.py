...
class EntityDisambiguationGraph(object):
    ...
    def __init__(self, kb_name, table_number, table, candidates, graph_path, infobox_property, abstracts, disambiguation_output_path):
        ...
        self.iterations = 500
        self.delta = 0.0001
        self.alpha1 = 0.3333333
        self.beta1 = 0.3333333
        self.alpha2 = 0.5
        self.beta2 = 0.5
        self.bonus = 0.0
    ...
    # 获取一个 entity e 的流行度，YLZ-2020-12-9，需要配代理
    def get_entity_searchresult(self,e):
        try:
            s = requests.session()
            ...
            r=s.get(url,proxies=proxies, verify=False,timeout=3)
            return json.loads(r.text)["query"]["searchinfo"]["totalhits"]
        except Exception as ee:
            return self.get_entity_searchresult(e)
            ...
            for c in range(nCol):
                ...
                if flag_NIL == False:
                    ...
                    for candidate in mention_candidates:
                        candidate_index += 1
                        EDG.add_node(entity_counter, type='eNode', candidate=candidate, index=candidate_index, mNode_index=mention_counter, probability=float(0), context=[], disambiguation='',searchresult=float(0),popular=float(0))
                        ...
                        #YLZ 添加流行度先验知识#2020-12-9
                        EDG.node[entity_counter]['searchresult'] = self.get_entity_searchresult(candidate)
                        popularcount+=EDG.node[entity_counter]['searchresult']
                    ...
                    for candidate in mention_candidates:
                        EDG.node[temp]['popular'] = EDG.node[temp]['searchresult']/float(popularcount)
                        temp+=1
                mention_counter += 1
    ...
    def strSim(self, m, e):
        ...
        if self.kb_name == 'baidubaike':            # 完整的实体，包括消岐义内容 real_entity[disambiguation]
            ...
            real_entity = split[0]                  # 真实的实体 (unicode)，去除了消岐义内容 real_entity
             ...
             # 真实的实体 (unicode)，去除了消岐义内容 real_entity
    ...
    def SR_me(self, m, e):
        alpha1 = self.alpha1
        beta1 = self.beta1
        sr_me = 0.99 * (alpha1 * self.strSim(m, e) + beta1 * self.contSim_me(m, e) +alpha1 * self.EDG.node[e]['popular']) + 0.01
        return sr_me
